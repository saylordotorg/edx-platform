...
class TestMixedModuleStore(object):
    ...
    def test_get_courses_for_wiki(self):
        ...
        """
        toy_courses = self.store.get_courses_for_wiki('toy')
        assert_equals(len(toy_courses), 2)
        for course in toy_courses:
            assert_equals(course.wiki_slug, 'toy')

        courses = self.store.get_courses_for_wiki('no_such_wiki')
